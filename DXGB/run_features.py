...
import DXGB.CyclicPoseFragmentation_noH
from DXGB.CyclicPoseFragmentation_noH import runMethod
import DXGB.get_fragment_Vina
from DXGB.get_fragment_Vina import generate_data
from DXGB.get_fragment_Vina import run_Vina_Fragment
    ...
    if decoy:
        ...
        for idx, decoy in enumerate(decoy_list):
            try:
                ...
                print("Problem fragments" + str(idx))
                continue
        out_core.close()
        out_side.close()
        out.close()
...
def get_input_decoy(datadir, datadir_decoy, fn, ligname):
    ...
    if decoy_file in os.listdir("../"):
        ...
        num = 20
        ### Since in flex docking, we have to use name of each atom to reorder, we need to use rename mol2 file as reference not initial mol2 file ###
        ### only mol2 file can be used as rdkit ligand ref, since sdf has no atom name ###
        rewrite_type = "name"
        ref_ligand_rdkit = fn + "_ligand_rename.mol2"
        
    decoy_list = [fn + "_" + str(i) + "_decoy.mol2" for i in range(1,num + 1)]
    rewrite_decoy(ref_ligand_rdkit, decoy_list, ref_fmt, rewrite_type)
    rewrite_decoy(ref_ligand_pdb, decoy_list, "pdb", rewrite_type)
...
def run_Vina_features(datadir, d_type, fn, inpro, inlig, decoy = False, decoy_list = None):
    ...
    if decoy:
        ...
        for decoy in decoy_list:
            try:
                ### if Vina calculation has problem, just skip this decoy ###
                idx = decoy.split("_")[1]
...
def run_dE_features(datadir, fn, inlig_rdkit, decoy = False, decoy_rdkit_list = None, rewrite = False):
    ...
    if decoy:
        ...
        for decoy in decoy_rdkit_list:
            try:
                ### if dE calculation has problem, just skip this decoy ###
                idx = decoy.split("_")[1]
