    ...
    if water_type == "rbw":
        ...
        if rewrite:
            ...
            if inpro_pro.split(".")[0] + "_BW.pdb" not in os.listdir(datadir):
                ...
                cal_BW(out_total,fn,inprot_pro,inlig,inpro_water,datadir, Feature = False)
                out_total.close()
                print("Finish generate RW(BW)")
            else:
                print("Use previous copy RW(BW)")

    elif water_type == "rw":
        print("Receptor Water: recalculate by RW")
        if rewrite:
            get_Crw(fn,inpro_pro,inpro_water,datadir)
            print("Finish generate RW")
        else:
        ...
        if rewrite:
            ...
            if inpro_pro.split(".")[0] + "_RW.pdb" not in os.listdir(datadir):
                ...
                print("Use previous copy RW(PW)")
    elif water_type == "bw":
        print("Receptor Water: recalculate by BW")
        if rewrite:
            out_total = open("Feature_BW_initial.csv","w")
            cal_BW(out_total,fn,inprot_pro,inlig,inpro_water,datadir, Feature = False)
            out_total.close()
...
def feature_calculation_ligand(datadir,fn, inlig_pdb, inlig_rdkit, inpro_pro, water_type, opt_type):
    ...
    ################################
    if opt_type == "rbwo":
        d_type = {"":[inpro_C, inlig_C],"_min": [inpro_C, inlig_Co],"_min_RW": [inpro_Crw, inlig_Crwo], "_min_BW":[inpro_Cbwo, inlig_Cbwo]}
        ### get bw ###
        for st in ["_BW","_RW"]:
            out_total = open(os.path.join(datadir,"Feature_BW_min" + st + ".csv"),"w")
        ...
        d_type = {"_min_BW": [inpro_Crw, inlig_Crwo]}
        ### get bw ###
        out_total = open(os.path.join(datadir,"Feature_BW_min_BW.csv"),"w")
        out_total.write("pdb,Nbw,Epw,Elw\n")
        cal_BW(out_total,fn,inpro_pro,inlig_Crwo,inpro_Crw,datadir)
        out_total.close()
        print("Finish Bridging Water Feature Calculation")
        ...
        out_total = open(os.path.join(datadir,"Feature_BW_min_PW.csv"),"w")
        out_total.write("pdb,Nbw,Epw,Elw\n")
        cal_BW(out_total,fn,inpro_pro,inlig_Crwo,inpro_Crw,datadir)
        out_total.close()
        print("Finish Bridging Water Feature Calculation")
    elif opt_type == "o":
        d_type = {"_min":[inpro_C, inlig_C]}
    else:
...
def run_features(datadir, datadir_pro, datadir_decoy, fn, pro, water_type = "rw", opt_type = "wo", rewrite = False, decoy = False, ligname = False):
    ...
    if water_type != "n":
        if not decoy:
            prepare_rw_receptor(datadir, fn, inpro_pro, inpro_water, inlig_pdb, water_type, rewrite)
        else:
