...
def runMSMS(inprot, inlig, MSMSDIR = '.'):
    ...
    if (os.path.isfile('p_sa.area') and os.path.isfile('pl_sa.area')) == False:
    ...
    df.columns = ['atm','pharma','p','c']
    os.chdir('../')
    os.system('rm -rf tmp_dp')
    os.chdir(olddir)
    return df
