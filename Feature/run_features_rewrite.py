...
import Get_Fragment_Vina58_rewrite 
from Get_Fragment_Vina58_rewrite import generate_data
from Get_Fragment_Vina58_rewrite import run_Vina_Fragment
...
def run_fragments(fn, datadir, inlig, water = True, decoy = False):
    '''
    ...
    if decoy:
        ...
        ### For CASF bechmark 
        run_Vina_Fragment(fn, datadir, datadir_frag, min = False, min_RW = False, RW = False, decoy = True)
        ...
        for i in range(decoy_num + 1):
            ...
            num_frags = generate_data(fn,i,datadir)
        out.write(fn + "," + str(num_frags) + "\n")
        out.close()
    if water:
        run_Vina_Fragment(fn, datadir, datadir_frag, min = True, min_RW = True, RW = False, decoy = False)
        out = open(os.path.join(datadir,"NumFrags.csv"),"w")
        out.write("pdb,num_frag\n")
        for i in range(3):
        ...
        run_Vina_Fragment(fn, datadir, datadir_frag, min = True, min_RW = False, RW = False, decoy = False)
        out = open(os.path.join(datadir,"NumFrags.csv"),"w")
        out.write("pdb,num_frag\n")
        for i in range(2):
...
def run_features(datadir,fn, rw = False, water = True, decoy = False):
    ...
    for i in d_type.keys():
        ...
        ### get ion ###
        outfile = open(os.path.join(datadir,"Num_Ions" + ".csv"),"w")
    ...
    ### run fragments ###
    run_fragments(fn, datadir, inlig_rdkit, water = water, decoy = decoy)
