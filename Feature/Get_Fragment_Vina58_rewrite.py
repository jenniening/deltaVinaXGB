...
def get_ref(fn, datadir, min = True, min_RW = True, RW = True, decoy = False):
    '''
    Get reference file
    ...
    if RW:
        ref_list.append(os.path.join(datadir,fn +"_ligand_Vina58.pdbqt"))
    if decoy:
        num = [line for line in open(os.path.join(datadir, "NumMol2.log")) ][0].strip()
        for i in range(1,num + 1):
            ref_list.append(os.path.join(fn + "_decoys_" + str(i) + ".pdb"))

    return ref_list

def get_prot(fn,datadir, min = True, min_RW = True, RW = True, decoys = False):
    '''
    Get protein pdbqt
    ...
    if decoy:
        num = [line for line in open(os.path.join(datadir, "NumMol2.log")) ][0].strip()
        for i in range(1,num + 1):
            ref_list.append(os.path.join(datadir,fn + "_protein_Vina58.pdbqt"))
...
def run_Vina_Fragment(fn, datadir, datadir_frag, min = True, min_RW = True, RW = True, decoy = False):
    
    # get ref_lig name
    ref_list = get_ref(fn,datadir,min, min_RW, RW, decoy = decoy)
    # get protein pdbqt
    prot_list = get_prot(fn,datadir,min,min_RW, RW, decoy = decoy)
