...
def get_ref_infor(ref_list, noname = True):
    '''
    ...
    if noname:
        ...
        ref_old = ref_list[0].split(".")[0][0:-7] + ".mol2"
...
def get_ref(fn, datadir, min = True, min_RW = True, RW = True):
    '''
    Get reference file
    ...
    if not os.path.isfile(os.path.join(datadir,fn + "_ligand_Vina58.pdbqt")):
        inlig = os.path.join(datadir,fn +"_ligand.mol2")
...
def get_prot(fn,datadir, min = True, min_RW = True, RW = True):
    '''
    Get protein pdbqt
...
def get_frag(fn,frag_dir):
    ...
    # BACE_1
    #frag_list = [os.path.join(frag_dir,'%s_frag%02d.pdb'%(fn, i) )for i in range(1,frag_num + 1)]
    # CatS_1
...
def run_Vina_Fragment(fn, datadir, datadir_frag, min = True, min_RW = True, RW = True):
    
    # get ref_lig name
    ref_list = get_ref(fn,datadir,min, min_RW, RW)
    # get protein pdbqt
    prot_list = get_prot(fn,datadir,min,min_RW, RW)
    # get previous ligand pdbqt 
    ref_infor = get_ref_infor(ref_list,noname = True)
    # get fragments name
...
def generate_data(df,data_type,datadir):
...
def combine_data(fn,datadir,data_type, previous_file, outfile_core, outfile_side):
    # Get outfile_core and outfile_side
    ...
    if os.stat(infile).st_size != 0:
        ...
        df.columns = columns
        generate_data(df,data_type,datadir)
        ...
        if len([line for line in open(infile)]) != 1:
            ...
            outfile_side.write(",".join([fn] + ["0" for i in range(59)]) + "\n")          
    else:
        for line in open(previous_file):
            if line.split(",")[0] == fn:
                outfile_core.write(fn + "," + ",".join(line.split(",")[0:60]) + '\n')
        outfile_side.write(",".join([fn] + ["0" for i in range(59)]) + "\n")
...
if __name__ == "__main__":
    ...
    for fn in pdblist:
        ...
        #run_Vina_Fragment(fn,datadir_current,fragdir_current, True, True, True)

        combine_data(fn,fragdir_current, data_type, previous_file, outfile_core, outfile_side)
    outfile_core.close()
    outfile_side.close()
    #### -------------CAS -------------------------
