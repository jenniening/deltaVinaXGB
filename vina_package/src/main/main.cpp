...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
	...
	if(score_only) {
		...
		//log << "Number of Terms: " <<  term_values.size() << '\n';
		log << std::fixed;
		for (unsigned i=0; i < term_values.size(); ++i){
			log << "Term" << i+1 << " " << std::setprecision(5) << term_values[i] << '\n';
		}

		conf_independent_inputs in(m);
		log << "Term53  " << in.num_tors << "\n" ;
		log << "Term54  " << in.num_rotors << "\n" ;
		log << "Term55  " << in.num_heavy_atoms << "\n" ;
		log << "Term56  " << in.num_hydrophobic_atoms << "\n" ;
		log << "Term57  " << in.ligand_max_num_h_bonds << "\n" ;
		log << "Term58  " << in.ligand_lengths_sum << "\n" ;
